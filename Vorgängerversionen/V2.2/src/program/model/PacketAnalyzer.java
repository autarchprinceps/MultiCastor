...
public class PacketAnalyzer {
	...
	public PacketAnalyzer(MulticastData multicastData, Logger logger){
		if (multicastData==null){
			System.out.println("B�ser Fehler!!! multicastData ist null im PacketAnalyzer."); //$NON-NLS-1$
		}
		mcData = multicastData;
		if(logger == null){
			System.out.println("B�ser Fehler!!! Message Queue ist null im PacketAnalyzer."); //$NON-NLS-1$
		}
		this.logger = logger;
...
//		System.out.println("DEBUG: analyzer begin");
		if(mcPacket.length!=length){
			logger.log(Level.SEVERE, "Invalid Packet Receivied in analysePacket()"); //$NON-NLS-1$
			return;
		}
		...
		if(hostID.equals("Hirschmann IP Test-Multicast")){ //$NON-NLS-1$
			if(check){
				logger.log(Level.INFO,"Muahaha - Pakete vom Hirschmann Tool entdeckt."); //$NON-NLS-1$
				check = false;
			}
			hostID="Hirschmann-Tool SenderID:"; //$NON-NLS-1$
		}
...
//			System.out.println("jitterv4");
	...
	public void analyzePacketOnce(byte[] mcPacket){
		...
		if(mcPacket.length!=65575){
			logger.log(Level.SEVERE, "Invalid Packet Receivied in analysePacketOnce()"); //$NON-NLS-1$
			return;
		}
		...
		if((hostID!=null)&&(hostID.equals("Hirschmann-Tool SenderID:"))){ //$NON-NLS-1$
