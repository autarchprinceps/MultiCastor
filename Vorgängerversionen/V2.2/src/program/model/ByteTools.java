...
/**
 * Diese Klasse stellt static Methoden zur Verf�gung, um mit Byte-Arrays zu arbeiten
 * @author jannik
 */
public class ByteTools {
	...
	public static byte[] convertToByte(boolean theBoolean){
		if(theBoolean) return new byte[]{						//true: mit Einsen f�llen
							(byte) 255,
							(byte) 255,
							(byte) 255,
							(byte) 255
						};
		else			return new byte[]{						//false: mit Nullen f�llen
							(byte) 0,
							(byte) 0,
							(byte) 0,
							(byte) 0
	...
	/**
	 * Errechnet eine CRC-CCITT (CRC-16) Pr�fsumme des �bergebenen byte-Arrays.
	 * Der Initialwert der Pr�fsumme ist 0xFFFF (11111111 11111111).
	...
	public static byte[] crc16(byte[] byteArray){
        ...
        //for-Schleife �ber alle bytes
        for (int i=0;i<byteArray.length;i++){
        	mybyte = byteArray[i];
        	//for-Schleife �ber alle Bits des jeweiligen bytes
            for (int j = 0; j < 8; j++) {
                if(	((mybyte   >> (7-j) & 1) == 1)
                  ^ ((crc 	   >>   15  & 1) == 1))
                {
	...
	/**
	 * Wandelt in ein Int um.
	 * Die Bitoperatoren w�rden ohne die Konvertierung der Bytes im
	 * 32-bit Bereich arbeiten, das w�rde hier zum falschen Ergebnis f�hren.
