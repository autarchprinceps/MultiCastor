...
public class PanelGraph extends JPanel {
  private int 		maxY			= 100,
					numberOfValues  = 62,
					dataPointer 	= 0;
  private Dimension panelSize       = new Dimension(740,100);
  private int[] 	data;
  private boolean	staticScale		= true;
  private int		dynScaleCount	= 0;

  //Beschriftungs- und Anzeigewerte Variablen
  private String 	lblX			= "", //$NON-NLS-1$
                 	lblY			= ""; //$NON-NLS-1$
  private int 		actualValue		= 0;
  Font 				graphFont 		= new Font("SansSerif", Font.PLAIN, 9); //$NON-NLS-1$
...
private void paintContent(Graphics g){
    ...
    for(int i=6;i>=0;i--){
      ...
      //X-Achse beschriften
      if(time==0)	g.drawString(time+"", x2-((int)x+2), 88); //$NON-NLS-1$
      else			g.drawString(time+"", x2-((int)x+5), 88); //$NON-NLS-1$
    ...
    g.drawString(maxY+"",	 1, 19); //$NON-NLS-1$
    g.drawString(maxY/2+"",  1, 49); //$NON-NLS-1$
    g.drawString("0", 		12, 79); //$NON-NLS-1$
    ...
    if(lblY.length()*5+35<x2)					g.drawString(Messages.getString("PanelGraph.1") + actualValue, (x2-70), 9); //$NON-NLS-1$
