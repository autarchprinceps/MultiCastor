...
import multicastor.data.MulticastData.Source;
import multicastor.data.MulticastData.senderState;

/**
 * PacketAnalyzer wertet die Pakete fuer den Empfaenger aus. Er erstellt auch
 * die Werte fuer die Tabelle.
 */
public class PacketAnalyzer {
	...
	private final MulticastData mcData;
	...
	private final byte[] snippetForChecksum = new byte[42];
	...
	public void analyzePacket(final byte[] mcPacket) {
		...
		final byte[] snippet = new byte[25];
		final byte[] randSnippet = new byte[4];
		final byte[] intSnippet = new byte[4];
		final byte[] shortSnippet = new byte[2];
		final byte[] longSnippet = new byte[8];
	 ...
	 *            Neuer PacketCount
	 */
	public void setInternerPacketCount(final int packetCount) {
	 ...
	 * @param lastActivated
	 */
	public void setLastActivated(final int lastActivated) {
	...
	public void update() {
		...
		// sorgt dafuer, dass jegliche Aenderungen erst mit einer Sekunde
		// Verzoegerungen
		...
		// Traffic
		final int traffic = mcData.getPacketRateMeasured()
		...
		// Added this for Lost packets cnt
