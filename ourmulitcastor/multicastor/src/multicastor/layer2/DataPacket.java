...
public class DataPacket {
	...
	/**
	 * @param source
	 *            is a byte array which contains the MAC address of the device
	 *            which will send the packet
	 * @param destination
	 *            is a byte array which contains the MAC address of the device
	 *            which should received the packet
	 * @param data
	 *            is a byte array which contains the data which should be send
	 * @return the data packet as a byte array
	 */

	public static byte[] getDataPacket(final byte[] source,
			final byte[] destination, final byte[] data) {
		final byte[] packet = new byte[14 + data.length];
