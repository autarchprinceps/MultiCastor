...
public class MldDocumentListener extends AbstractDocumentListener {
	PanelMulticastConfigNew panel;
	private ViewController ctrl;

	public MldDocumentListener(PanelMulticastConfigNew panel) {
		this.panel = panel;
	}
	...
	public void insertUpdate(DocumentEvent arg0) {
		...
		accepted = (InputValidator
				.checkMC_IPv6(panel.getTf_address().getText()) != null);
		...
		if (ctrl.getCurrentModus().equals(Modus.SENDER)) {
			...
			accepted = (InputValidator
					.checkIPv6(panel.getCb_interface().getSelectedItem().toString()) != null);
			}else{
				accepted = false;
			}
			if (!accepted && buttonEnabled) {
			...
			accepted = (InputValidator.checkIPv6PacketLength(panel
		...
		// Hier die anderen Felder einfï¿½gen
