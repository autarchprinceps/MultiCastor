...
public class Multicast_SenderTC {
	...
	MulticastReceiverLayer3 myMcReceiver;
	Queue<String> messages       = new LinkedList<String>(),
				  messagesRec	 = new LinkedList<String>();
	IgmpMldData mcData 		 = new IgmpMldData(ProtocolType.IGMP),
				  mcDataReceived = new IgmpMldData(ProtocolType.IGMP);
	...
	public void testIpv4(){
		...
		Vector<InetAddress> sourceIPv4 = new Vector<InetAddress>();
		for (NetworkInterface inetIf : NetworkAdapter.getipv4Adapters()) {
			Enumeration<InetAddress> inetAd = inetIf.getInetAddresses();
			while (inetAd.hasMoreElements()) {
				InetAddress inetAddress = (InetAddress) inetAd.nextElement();
				if(inetAddress instanceof Inet4Address){
					sourceIPv4.add(inetAddress);
				}
			}
		}
...
//ANALYSE-ENDE
	...
	private void runSenderAndReceiver(){
		...
		myMcSender = new MulticastSenderIgmpMld(mcData, logger);
		myMcReceiver = new MulticastReceiverLayer3(mcDataReceived, logger);
	...
	private void prepareSampleDataV6(){
		mcData = new IgmpMldData(ProtocolType.MLD);
		mcDataReceived = new IgmpMldData(ProtocolType.MLD);
		...
		//Typ
		mcData.setProtocolType(ProtocolType.UNDEFINED);
		mcDataReceived.setProtocolType(ProtocolType.UNDEFINED);
	}
